...
import { Plugin, TFile, TFolder, FuzzySuggestModal, FuzzyMatch, Editor } from 'obsidian';
...
export default class HandwritingPlugin extends Plugin {
	...
	// Mappa embedId → azioni expand/collapse usata dal menu "⋮" di Obsidian.
	...
	async loadSettings() {
		...
		if (!this.settings.storageNamespaceMigrated) {
			if (this.settings.svgFolder === '_handwriting') this.settings.svgFolder = '_inline_handwriting';
			this.settings.storageNamespaceMigrated = true;
...
class SvgReferenceSuggest extends FuzzySuggestModal<TFile> {
	...
	// Restituisce tutti gli SVG nella cartella configurata.
	getItems(): TFile[] {
